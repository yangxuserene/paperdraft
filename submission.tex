...
Network contention between concurrently running jobs on HPC system is one of the primary causes of performance variability. Optimizing job allocation and avoiding network sharing are proven techniques to alleviate network contention and performance degradation. In this work, we choose three representative HPC applications from DOE Design Forward Project and conduct detailed analysis about intra- and inter-job interference with them. By scrutinizing the communication behaviors of these applications, we identify the relation between applications' communication pattern and the possible interference when they running with different allocations. We propose a novel and flexible strategy that makes allocation with awareness of job's communication pattern. The new strategy can effectively alleviate the performance degradation caused by interference.
