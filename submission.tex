...
\begin{figure}[h!]
    ...
    \end{subfigure}%
   \caption{Indication of multiple jobs running on Cray system. Many jobs runninng concurrently on the system. Different jobs represented by specific colors.}
   \label{fig: overview}
\end{figure}
...
\begin{figure}[h!] 
  \centering
  \includegraphics[width=0.38\textwidth]{figs/appstudy/amg/amg_pip}
...
\begin{figure}[h!] 
  \centering
  \includegraphics[width=0.38\textwidth]{figs/appstudy/cr/cr_pip}
...
\begin{figure}[h!] 
  \centering
  \includegraphics[width=0.38\textwidth]{figs/appstudy/mg/mg_pip}
...
The torus network performance is determined by its dimensionality and link bandwidth. As the increase of the dimensionality of torus netowrk, so does the number of links connected with each node. The increased aggregated bandwidth of each node will definitely reduce the data transfer time of each rank in the application. Figure \ref{fig: dimenstudy} shows the performance of the AMG, CrystalRouter and MultiGrid on a 2K node torus network model with a 3D torus (16x16x8), a 5D torus (8x4x4x4x4), and a 7D torus (4x4x4x4x2x2x2). The bandwidth between nodes is 2GiB/s in one direction, thus, the aggregated bandwidth is 12GiB/s per node in 3D torus, 20GiB/s per node in 5D torus, and 28GiB/s per node in 7D torus. As we can see from these figures, communication time of both applications decrease as the dimensionality of the network increase. The increased aggregated bandwidth of each node can accelerate the transfer of data. This can justify the fidelity and consistency of the torus network model. The same pattern can be found in Figure \ref{fig:bandwidthstudy}, as the bandwidth increase from 2GiB/s, to 4GiB/s and 8GiB/s in the 5D torus network, the data transfer time can be greatly reduced.

\begin{figure}[h!]
    \centering
    \begin{subfigure}[t]{0.2\textwidth}
        \centering
        \includegraphics[height=1.2in]{figs/dimenstudy/avgtime}
        \caption{Normalized Average Time.}
        \label{fig:1sub1}
    \end{subfigure}%
    \hspace{1em}%
    \begin{subfigure}[t]{0.2\textwidth}
        \centering
        \includegraphics[height=1.2in]{figs/dimenstudy/maxtime}
        \caption{Normalized Max Time.}
        \label{fig:1sub2}
    \end{subfigure}%
   \caption{Normalized time spent to send data over 3D, 5D and 7D torus networks by AMG1728, CrystalRouter1000, MultiGrid1000.}
   \label{fig: dimenstudy}
\end{figure}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[t]{0.2\textwidth}
        \centering
        \includegraphics[height=1.3in]{figs/bandwidthstudy/avgtime}
        \caption{Normalized Average Time.}
        \label{fig:2sub1}
    \end{subfigure}%
    \hspace{1em}%
    \begin{subfigure}[t]{0.2\textwidth}
        \centering
        \includegraphics[height=1.3in]{figs/bandwidthstudy/maxtime}
        \caption{Normalized Max Time.}
        \label{fig:2sub2}
    \end{subfigure}%
   \caption{Normalized time spent to send data over 5D torus with 2GB/s, 4GB/s and 8GB/s bandwidth by AMG1728, CrystalRouter1000, MultiGrid1000.}
   \label{fig:bandwidthstudy}
\end{figure}

%\begin{figure}[h!] 
%  \centering
%  \includegraphics[width=0.38\textwidth]{figs/bandwidthstudy/maxtime}
%   \caption{The Normalized Max Time spent to send data over 5D torus with 2GB/s, 4GB/s and 8GB/s bandwidth by AMG1728, CrystalRouter1000, MultiGrid1000.}
%   \label{fig: bandwidthstudy-maxtime}
%\end{figure}
%
%\begin{figure}[h!] 
%  \centering
%  \includegraphics[width=0.38\textwidth]{figs/bandwidthstudy/avgtime}
%   \caption{The Normalized Average Time spent to send data over 5D torus with 2GB/s, 4GB/s and 8GB/s bandwidth by AMG1728, CrystalRouter1000, MultiGrid1000.}
%   \label{fig: bandwidthstudy-avgtime}
%\end{figure}


\section{Interference Analysis}
\textbf{presents results of single app running with con/non-cont allocation}
In this section, we will explore the communication interference. First, we will analysis the application's intra-job interference by assigning each job different shapes of allocation. All these allcoation are contigous in their dimensionality.
...
To study the intra-job interference of each job, we simulate each job running on 3D torus network with different allocation shapes. We assign each application with three different shapes allocation, i.e. 3D balanced-cube, 3D unbalanced-cube, 2D mesh. Figure \ref{figs/cont/allocation} shows some possible allocations for a 64-node job. 3D balanced allocation is $\{4,4,4\}$ as red, 3D unbalanced allocation is $\{8,4,2\}$ as green, 2D allocation is $\{8,8,1\}$ as blue.

Figure \ref{fig: differentshape-time} shows the data transfter time of three applications running with three different allocation ships.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[t]{0.2\textwidth}
        \centering
        \includegraphics[height=1.2in]{figs/intra-job/shapestudy/avgtime}
        \caption{Normalized Average Time.}
        \label{fig:shape-sub1}
    \end{subfigure}%
    \hspace{1em}%
    \begin{subfigure}[t]{0.2\textwidth}
        \centering
        \includegraphics[height=1.2in]{figs/intra-job/shapestudy/maxtime}
        \caption{Normalized Max Time.}
        \label{fig:shape-sub2}
    \end{subfigure}%
   \caption{Normalized time spent to send data by AMG216, CrystalRouter100, MultiGrid125 with 2D, 3D-unbalanced, 3D-balanced allocation.}
   \label{fig: differentshape-time}
\end{figure}
...
%\end{figure}
Then, we simulate each application running on 3D torus with contiguous allocation and non-contiguous allocation. For non-contiguous allocation we set the chunk size to 10 in order to preserve application's locality. Figure \ref{fig: non-cont-vs-cont} shows the data transfer time of application running with contiguous and non-contiguous allocation.

\begin{figure}[h!] 
  \centering
  \includegraphics[width=0.38\textwidth]{figs/intra-job/non-cont-vs-cont}
   \caption{Three Applications running exclusively on 3D torus with contigous and non-contiguous allocation}
   \label{fig: non-cont-vs-cont}
\end{figure}
...
\label{sec: interjob ference study}
...
%    \begin{subfigure}[t]{0.2\textwidth}
%        \centering
%        \includegraphics[height=1.2in]{figs/inter-job/noncont/exclusive}
%        \caption{"Exclusive Mode".}
%        \label{fig:noncont_sub2}
%    \end{subfigure}
    \caption{(a) Illustrates the contiguous allocation in different shape. (b) Illustrates the non-contiguous allocation with chunk of size 4.}
\end{figure}

In order to analysis the interference between concurrently runing jobs, we simulate three application running concurrently on the same 3D network.  We assign each application with a non-contiguous set of small chunk of nodes, 'chunk' refers to a small number(like 4) of adjacent nodes as shown in Figure \ref{fig:noncont_sub1}. The chunk belongs to one application is next to each other. The "chunk" size is critical to the application's performance. Figure\ref{fig: chunk_size} shows the results of applications' data transfer time with different chunk size. 


\begin{figure}[h!] 
  \centering
  \includegraphics[width=0.38\textwidth]{figs/inter-job/noncont/chunk_size}
   \caption{The Normalized Time spent to send data by AMG216, CrystalRouter100, MultiGrid125 with different chunk size.}
